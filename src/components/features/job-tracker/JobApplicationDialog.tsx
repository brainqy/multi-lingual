...
interface JobApplicationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (applicationData: Partial<Omit<JobApplication, 'id'>>, interviews: Interview[], notes: string[]) => void;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit(onValidSubmit)} className="flex-grow overflow-hidden flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <Card className="p-4 bg-secondary/50">
                          ...
                          <div className="space-y-3">
                              ...
                              <Button type="button" onClick={handleAddInterview} className="w-full">Add Interview</Button>
                          </div>
                      </Card>
                      <div className="space-y-2">
                         <h4 className="font-semibold">Scheduled Interviews</h4>
                         <ScrollArea className="h-72 pr-3 border rounded-md">
                              {currentInterviews.length > 0 ? (
                              <ul className="space-y-2 p-3">
                                  {currentInterviews.map(interview => {
                                  const interviewDate = typeof interview.date === 'string' ? parseISO(interview.date) : interview.date;
                                  return (
                                      <li key={interview.id} className="p-2 border rounded-md flex justify-between items-center text-sm bg-background">
                                      <div>
                              ...
                              ) : <p className="text-sm text-muted-foreground text-center p-4">No interviews scheduled yet.</p>}
                         </ScrollArea>
                      </div>
                  </div>
                </TabsContent>
                <TabsContent value="notes">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Textarea value={newNoteContent} onChange={e => setNewNoteContent(e.target.value)} placeholder="Add a new note..." rows={4}/>
                        <Button type="button" onClick={handleAddNote} className="w-full">Add Note</Button>
                      </div>
                      <div className="space-y-2">
                        <h4 className="font-semibold">Existing Notes</h4>
                        <ScrollArea className="h-64 pr-3 border rounded-md">
        ...
        <DialogFooter className="pt-4 border-t shrink-0 flex justify-between w-full">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" onClick={handleSubmit(onValidSubmit)} className="bg-primary hover:bg-primary/90 text-primary-foreground">{t("jobTracker.dialog.save", { default: "Save" })}</Button>
          </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
