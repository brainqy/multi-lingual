...
const CreativeTemplate = ({ data }: TemplateProps) => {
  ...
  return (
    <div className="p-6 font-sans text-gray-800 bg-white flex">
      {/* Left Sidebar */}
      <div className="w-1/3 bg-gray-100 p-4 rounded-l-lg">
        <h1 className="text-2xl font-bold text-gray-900">{data.header.fullName}</h1>
        <p className="text-md text-primary font-medium mb-4">{data.experience[0]?.jobTitle || 'Aspiring Professional'}</p>
        
        <div className="space-y-3 text-xs">
          {data.header.email && <div className="flex items-center gap-2"><Mail className="h-3 w-3 text-primary"/><span>{data.header.email}</span></div>}
          {data.header.phone && <div className="flex items-center gap-2"><Phone className="h-3 w-3 text-primary"/><span>{data.header.phone}</span></div>}
          {data.header.address && <div className="flex items-center gap-2"><Home className="h-3 w-3 text-primary"/><span>{data.header.address}</span></div>}
          {data.header.linkedin && <div className="flex items-center gap-2"><Linkedin className="h-3 w-3 text-primary"/><a href={data.header.linkedin} className="text-blue-600 hover:underline">LinkedIn</a></div>}
        </div>
        
        {data.skills.length > 0 && (
          <div className="mt-4">
            <h2 className="text-sm font-bold uppercase text-primary tracking-wider mb-1">Skills</h2>
        ...
        {data.education.length > 0 && (
          <div className="mt-4">
            <h2 className="text-sm font-bold uppercase text-primary tracking-wider mb-1">Education</h2>
      ...
      <div className="w-2/3 p-6">
        {data.summary && (
          <div className="mb-4">
            <h2 className="text-lg font-bold text-primary tracking-wide border-b-2 border-primary/30 pb-1 mb-2">Profile</h2>
        ...
        {data.experience.length > 0 && (
          <div className="mb-4">
            <h2 className="text-lg font-bold text-primary tracking-wide border-b-2 border-primary/30 pb-1 mb-2">Experience</h2>
        ...
        {data.additionalDetails && Object.values(data.additionalDetails).some(val => val && val.length > 0) && (
            <div className="mt-4">
                <h2 className="text-lg font-bold text-primary tracking-wide border-b-2 border-primary/30 pb-1 mb-2">More</h2>
