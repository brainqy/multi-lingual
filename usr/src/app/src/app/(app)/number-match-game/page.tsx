...
export default function NumberMatchGamePage() {
  ...
  const handlePlay = async () => {
    if (isRolling || !isGameActive || !user) {
      if (!isGameActive) {
    ...
    if (!gameStarted) {
      ...
      if (!userWallet || userWallet.coins < GAME_COST) {
        toast({ title: "Insufficient Coins", description: `You need ${GAME_COST} coins to play.`, variant: "destructive" });
        return;
      }

      await updateWallet(user.id, { coins: userWallet.coins - GAME_COST }, "Number Match Game Fee");
      setGameStarted(true);
    ...
    const finishRoll = async () => {
      ...
      if (newNumber === WINNING_NUMBER) {
        ...
        const userWallet = await getWallet(user.id);
        if (userWallet) {
            await updateWallet(user.id, { coins: userWallet.coins + WIN_REWARD }, "Number Match Game Jackpot!");
        }
        ...
        if (newAttemptsLeft <= 0) {
          ...
          if (updatedTotalConsolation > 0) {
            const userWallet = await getWallet(user.id);
            if (userWallet) {
                await updateWallet(user.id, { coins: userWallet.coins + updatedTotalConsolation }, "Number Match Game Consolation Prize (Total)");
            }
          }
        } else {
