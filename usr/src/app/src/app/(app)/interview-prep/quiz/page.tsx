...
import { AlertCircle, Bookmark, Check, ChevronLeft, ChevronRight, Clock, Send, X, PieChart as PieChartIcon, BarChart2 as BarChart2Icon, ListChecks, Maximize, Minimize, Info, CheckSquare as CheckSquareIcon, Circle, Radio, Lightbulb, Share2 } from 'lucide-react';
import { sampleInterviewQuestions, sampleCreatedQuizzes, sampleUserProfile } from '@/lib/sample-data';
import type { InterviewQuestion, MockInterviewSession, CommunityPost } from '@/types';
...
export default function QuizPage() {
  ...
  }, [quizStarted, totalQuizTime, questions.length]);
  ...
  const handleShareToFeed = async () => {
    if (!quizResults || !quizMetadata) {
      toast({ title: "Error", description: "Quiz data is not complete for sharing.", variant: "destructive" });
      return;
    }
    const currentUser = sampleUserProfile;

    let postContent = `I just took the "${quizMetadata.topic}" quiz and scored ${quizResults.percentage.toFixed(0)}%! 🚀`;
    postContent += `\n\nIt's a great way to test your knowledge. I recommend trying it out in the Interview Prep Hub.`;
    postContent += `\n\n#InterviewPrep #QuizChallenge #${quizMetadata.topic.toLowerCase().replace(/\s+/g, '')}`;
    
    const newPostData: Omit<CommunityPost, 'id' | 'timestamp' | 'comments'> = {
      ...
      content: postContent,
      type: 'text',
      tags: ['InterviewPrep', 'QuizChallenge', quizMetadata.topic.toLowerCase().replace(/\s+/g, '')],
    ...
    if (newPost) {
        toast({
            ...
            description: "Your quiz achievement has been posted to the community feed.",
            duration: 5000,
        });
    } else {
        toast({ title: "Error", description: "Could not share to feed. Please try again.", variant: "destructive" });
    }
  };
  ...
  if (quizSubmitted && quizResults) {
    ...
    const topicsForRevision = Object.entries(quizResults.categoryStats)
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 space-y-6 bg-secondary/30 dark:bg-background">
        ...
        {topicsForRevision.length > 0 && (
          <Card className="w-full max-w-2xl p-6 shadow-xl">
            <CardHeader className="p-0 pb-4">
              <CardTitle className="text-xl font-semibold flex items-center gap-2">
                <Lightbulb className="h-5 w-5 text-primary"/> Topics for Revision
              </CardTitle>
              <CardDescription>
                Focus on these areas to improve your score next time.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0 flex flex-wrap gap-2">
              {topicsForRevision.map(topic => (
                  <Badge key={topic} variant="secondary" className="text-md px-3 py-1">{topic}</Badge>
              ))}
            </CardContent>
          </Card>
        )}
        ...
        <div className="w-full max-w-2xl flex flex-col sm:flex-row gap-2">
            <Button onClick={() => router.push('/interview-prep')} className="w-full sm:flex-1" variant="outline">
                Back to Interview Prep
            </Button>
            <Button onClick={handleShareToFeed} className="w-full sm:flex-1 bg-primary hover:bg-primary/90">
                <Share2 className="mr-2 h-4 w-4"/> Share Progress to Feed
            </Button>
        </div>
      </div>
    );
