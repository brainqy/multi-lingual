    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Star className="h-5 w-5 text-primary"/>{t("gamification.xpLevel.title")}</CardTitle>
          </CardHeader>
          <CardContent className="text-center">
            <p className="text-sm text-muted-foreground">{t("gamification.xpLevel.level")}</p>
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              <span>{t("gamification.xpLevel.xpToNextLevel", { count: xpForNextLevel - (user.xpPoints || 0), level: xpLevel + 1 })}</span>
            </div>
            <p className="text-sm font-semibold mt-4">{t("gamification.xpLevel.totalXp", { count: user.xpPoints || 0 })}</p>
          </CardContent>
        </Card>
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Flame className="h-5 w-5 text-primary"/>{t("gamification.dailyStreak.title")}</CardTitle>
          </CardHeader>
          <CardContent className="text-center">
            <p className="text-4xl font-bold text-primary">{user.dailyStreak || 0}</p>
            <p className="text-lg text-muted-foreground">{t("gamification.dailyStreak.dayStreak")}</p>
          </CardContent>
        </Card>
         <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Award className="h-5 w-5 text-primary"/>{t("gamification.badgesEarned.title")}</CardTitle>
          </CardHeader>
          <CardContent className="text-center">
            <p className="text-4xl font-bold text-primary">{earnedBadges.length}</p>
            <p className="text-lg text-muted-foreground">{t("gamification.badgesEarned.totalBadges")}</p>
          </CardContent>
        </Card>
      </div>
                 ...
                 <Tooltip key={badge.id}>
                  <TooltipTrigger asChild>
                   ...
                   <TooltipContent>
                    ...
                    <p className="text-xs text-muted-foreground">{badge.description}</p>
                    {badge.xpReward && <p className="text-xs text-yellow-500">+{badge.xpReward} XP</p>}
                     <p className="text-xs text-red-500 mt-1">{t("gamification.notYetEarned")}</p>
                  </TooltipContent>
                </Tooltip>
              ))}
            </div>
      ...
      <Card className="shadow-lg">
        ...
        <CardContent>
          {leaderboardUsers.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">{t("gamification.noLeaderboardData")}</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-[80px] text-center">{t("gamification.table.rank")}</TableHead>
                  <TableHead>{t("gamification.table.user")}</TableHead>
                  <TableHead className="text-right">{t("gamification.table.xpPoints")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  <TableRow key={lbUser.id} className={cn(index < 3 && "bg-secondary/50 font-semibold", lbUser.id === user.id && "bg-primary/10 border-l-2 border-primary")}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <span className={cn("font-medium", index < 3 && "text-primary")}>{lbUser.name} {lbUser.id === user.id && t("gamification.you")}</span>
                      </div>
                    </TableCell>
