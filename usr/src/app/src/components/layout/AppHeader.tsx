...
export function AppHeader() {
  ...
  const { t } = useI18n();
  const { user, logout, wallet, setStreakPopupOpen } = useAuth();
        ...
        <div className="hidden sm:flex h-10 items-center justify-end gap-4 border-t bg-secondary/30 px-4 md:px-6">
           <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="sm" className="h-auto px-2 py-1 text-sm font-medium text-muted-foreground" onClick={() => setStreakPopupOpen(true)}>
